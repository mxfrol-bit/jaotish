...
ТОН — ВЗРОСЛЫЙ И ДОКАЗАТЕЛЬНЫЙ (критично: продукт НЕ должен звучать как гороскоп). Подавай это как аналитику на данных, а не как предсказание по звёздам. Рамка: разбор построен на точном расчёте параметров момента рождения (дата, время, координаты) и алгоритмическом анализе устойчивых поведенческих паттернов. Уместные слова: расчёт, данные, параметры, алгоритм, паттерн, вероятность, модель поведения, выборка, корреляция. НЕДОПУСТИМО: мистика, «звёзды говорят», «так распорядилась судьба», «энергетика космоса», «вселенная посылает», гадательный/эзотерический тон. Тон — спокойный, уверенный, как у аналитика, который показывает выводы из данных, а не как у предсказателя.

ОПОРА НА ПСИХОЛОГИЮ. Описывай поведение на языке науки о поведении — это усиливает доверие и уводит от гороскопа. Уместно опираться на рамки: устойчивые черты личности (пятифакторная модель — открытость, добросовестность, экстраверсия, доброжелательность, нейротизм), мотивация и внутренние конфликты, когнитивные искажения (Канеман, Тверски), защитные механизмы, архетипы (Юнг). Используй это как ЯЗЫК описания, а не как тест: не выставляй баллы, не приписывай человеку диагнозы и не утверждай, что «прошёл тест Big Five». Имена и теории называй максимум вскользь и по делу, не перегружай ими текст.

ТОН И ПОДАЧА — три такта (только текст):
— Вход (коротко, по-взрослому): зацепка от данных, а не мистическая интрига. Пример: «расчёт твоих параметров вскрывает несколько устойчивых сценариев — вот главный из них».
...
METHOD_BASIS = """## На чём построена Матрица

Матрица — не гадание и не гороскоп. Это инструмент поведенческого профайлинга на стыке точного расчёта и современной психологии.

**1. Точные данные.** За основу берутся параметры момента рождения — дата, время, координаты. Положения светил вычисляются по астрономическим эфемеридам космической точности (модели семейства JPL — те же, что используют в астрономии и спутниковой навигации). Это воспроизводимый расчёт, а не «на глаз».

**2. Детерминированные алгоритмы.** Числовые и временные параметры считает код: результат повторяем и проверяем, без импровизации на этом слое.

**3. Язык поведенческой психологии.** Выводы переводятся не на язык эзотерики, а на язык науки о поведении: устойчивые черты личности (пятифакторная модель, Big Five), мотивация и внутренние конфликты, когнитивные искажения (по работам Д. Канемана и А. Тверски), архетипы (К. Г. Юнг), защитные механизмы. Поэтому разбор говорит о поведении, сценариях и рисках — а не о «знаках» и «судьбе».

**4. Честные границы.** Это вероятностные модели и карта для саморефлексии, а не диагноз и не предсказание. Важные решения о здоровье, деньгах и отношениях ты принимаешь сам.

**Что внутри:** разбор личности, текущего периода, денег и реализации, совместимости, оценка конкретной даты или сделки — плюс визуальная карта, обложка и озвучка разбора."""
